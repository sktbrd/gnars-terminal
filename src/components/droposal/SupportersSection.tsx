...
// Supporter Avatar component - Now optimized
...
export const SupportersSection: React.FC<SupportersSectionProps> = ({
  ...
  const fetchSize = totalSupply
    ? Math.min(Number(totalSupply), maxFetchSize)
    : maxFetchSize;

  const { supporters, loading, error, cached } = useSupporters({
    contractAddress,
    totalSupply,
    batchSize: fetchSize,
