...
const TransactionForm: React.FC<TransactionFormProps> = ({
  ...
  const handleChange = (field: string, value: string) => {
    if (field === 'amount') {
      ...
      (field === 'imageURI' || field === 'animationURI') &&
      type === 'DROPOSAL MINT'
    ) {
    ...
    if (fields.find((f) => f.name === field)?.validate) {
      const validation = fields.find((f) => f.name === field)?.validate!(
        value.replace(/,/g, '')
      );
      setErrors((prev) => ({
        ...prev,
        [field]: typeof validation === 'string' ? validation : '',
      }));
    }
  };
      ...
      {fields.map((field) => (
        ...
        >
          {field.type === 'file' ? (
            ...
            />
          ) : (field.name === 'imageURI' || field.name === 'animationURI') &&
            type === 'DROPOSAL MINT' ? (
            <div
              style={{
                display: 'flex',
